...
export class LoginComponent {
  ...
  onLogin(event: Event) {
    ...
    if (!this.credentials.username || !this.credentials.password) {
      this.toastService.showToast('กรุณากรอกข้อมูลให้ครบถ้วน', 'danger');
      return;
    }
      ...
      next: (res) => {
        if (res.success) {
          ...
          if (res.google2fa_enabled === 1) {
            ...
            this.toastService.showToast('กรุณากรอกรหัส OTP จากแอป Google Authenticator', 'success');
            ...
            this.toastService.showToast('รหัสผ่านถูกต้อง กรุณาตั้งค่าระบบความปลอดภัย 2FA', 'success');
          }
        }
      },
      error: (err) => {
  ...
  onVerifySetup2FA(): void{
    ...
    if (this.otpCode.length != 6) {
      this.toastService.showToast('กรุณากรอกรหัสความปลอดภัยให้ครบ 6 หลัก', 'danger');
      return;
    }
    this.isLoading.set(true);
    this.authService.verifySetup2FA(this.tempNationalId, this.otpCode).subscribe({
      next: (res) => {
        if (res.success) {
          this.toastService.showToast('เปิดใช้งานระบบ 2FA และเข้าสู่ระบบสำเร็จ', 'success');
          this.otpInputs.set(['', '', '', '', '', '']);
          this.otpCode = '';
          this.router.navigate(['/portal']); 
        }
  ...
  onVerifyDaily2FA(): void{
    ...
    this.authService.verifyDaily2FA(this.tempNationalId, this.otpCode, this.isRememberDevice()).subscribe({
      next: (res) => {
        if (res.success) {
          this.toastService.showToast('ยินดีต้อนรับเข้าสู่ระบบพอร์ทัล สสจ. นครศรีฯ', 'success');
          this.otpInputs.set(['', '', '', '', '', '']);
          this.otpCode = '';
          this.router.navigate(['/portal']); 
        }
